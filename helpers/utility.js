...
function getBalance() {
    ThirdPartyModel.find({}, function (err, found) {
        if (err) {
            console.log(err)
        } if (!found) {
        ...
        if (found) {
            found.map((a) => {
                var config = {
                    method: 'get',
                    url: `https://broker-api.sandbox.alpaca.markets//v1/trading/accounts/${a.acc_id}/account`,
                    headers: {
                        'Authorization': 'Basic Q0sxMDAxM0RXT0xNWkJJRThJQVE6Y1RsQ3kzQnpJVGZaazcwSXNDWGNrRURTTHBIVTZzOXBaQjRUZlhnYQ=='
                    }
                };
                ...
                axios(config)
                    .then(function (response) {
                        var dateArr = []
                        var amountArr = []
                        ThirdPartyModel.findOne({ acc_id: a.acc_id }, function (err, noel) {
                            if (err) {
                                console.log(res, err)
                            }
                            if (noel) {
                                dateArr.push(Date.now())
                                amountArr.push(response.data.portfolio_value || '0')
                                const updat = {
                                    dateArr: [...noel.dateArr, ...dateArr],
                                    amountArr: [...noel.amountArr, ...amountArr]
                                }
                                ThirdPartyModel.findOneAndUpdate({ _id: noel._id }, updat, { new: true }, function (err, updated) {
                                    if (err) {
                                        console.log(err)
                                    }
                                    if (updated) {
...
exports.CountriesAndNationality = [
    {
        "num_code": "4",
        "alpha_2_code": "AF",
        "alpha_3_code": "AFG",
